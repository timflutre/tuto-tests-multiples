...
$\mathcal{L}(\mu) = p(y_1 | \mu) = \frac{1}{\sqrt{2 \pi}} \exp \left( - \frac{(y_1 - \mu)^2}{2} \right)$
...
plot(x=x, y=dnorm(x=x, mean=2, sd=1), type="l", col="blue",
     main="", las=1, lwd=2,
     xlab="y", ylab="densité de probabilité")
points(x=x, y=dnorm(x=x, mean=5, sd=1), type="l", col="red", lwd=2)
abline(v=0, lty=2)
legend("topright", legend=c("mu=2", "mu=5"), col=c("blue", "red"), lty=1,
       bty="n", cex=1.5, lwd=2)
...
text(x=-2.1, y=0.05, labels=expression(paste("p(", y[1], " | ", mu == 1, ")",
  sep="")), cex=1.5)
```
...
> - probabilité critique: $p = P(Z \ge z | H_0)$
...
> 2. estimer chaque effet par maxlik, $\hat{\beta}_j$ (erreur standard $s_j$)

> 3. les transformer en scores standardisés, $z_j = \frac{\hat{\beta}_j}{s_j}$

> 4. calculer les valeurs critiques, $p_j$, en supposant $Z_j | H_0 \sim \mathcal{N}(0, 1)$
...
- $\hat{\beta}_j,s_j$: ashr (Stephens, dépôt GitHub)
